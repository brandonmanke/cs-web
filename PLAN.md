...
There is no `assets/` directory and no binary in the tree: every texture,
model, map and sound is generated by the code above.
...
- GPL engines (Quake, xash3d) are *behavior* references only — no code copied.
  ...
  fresh here.
- New art is generated in code, or original/CC0 with provenance recorded in the
  commit that adds it. There is no `assets/` directory and no binary in the
  tree; the reference packs that used to sit there were deleted once every
  model, texture and sound was procedural.
- `vite.config.ts` `publicDir` must never be repointed at a directory of
  binaries — Vite copies it verbatim into `dist/`.
