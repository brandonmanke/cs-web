...
//
// Roaming and pursuit follow hull-tested routes through the static world.
// Combat aim remains independent of movement, so a bot can face an opponent
// while taking a ramp or retreating behind cover to reload.
...
/** Check the whole footprint, including partial support along a ledge. */
bool footing_ahead(const PlayerEntity& e, float dx, float dz, float distance = 34.0F) {
  const Vec3 feet = feet_of(e);
  // Begin above an ascending ramp; a trace that starts inside it reports no hit.
  const Vec3 start = {feet.x + dx * distance, feet.y + kHullHalfHeightStand + 48.0F,
                      feet.z + dz * distance};
  const Vec3 end = {start.x, feet.y + kHullHalfHeightStand - 48.0F, start.z};
  const TraceResult floor = world_trace_hull(start, end,
      {kHullHalfWidth, kHullHalfHeightStand, kHullHalfWidth});
  return floor.hit && floor.normal.y >= kGroundNormalMinY;
}

void pick_goal(SimState& s, std::uint32_t index) {
  BotState& bot = s.players[index].bot;
  bot.goal_ticks = 1280U + rand_below(s, 256U);
  if (s.spawn_count == 0U || !nav_plan(s, index, true)) {
    bot.goal = s.players[index].move.origin;
    bot.path_count = bot.path_cursor = 0U;
  }
}
...
InputCommand bot_think(SimState& s, std::uint32_t index) {
  ...
  if (!engages) {
    ...
    if (!keep) {
      if (previous < kMaxPlayers) {
        ...
        bot.goal_ticks = 220U;
        if (bot.cover_ticks == 0U) nav_plan(s, index, false);
      }
  ...
  if (bot.navigation_ticks > 0U) --bot.navigation_ticks;
  if (bot.cover_ticks > 0U) --bot.cover_ticks;
  if (has_target && bot.navigation_ticks == 0U && bot.cover_ticks == 0U) {
    bot.navigation_ticks = 48U;
    const PlayerEntity& victim = s.players[bot.target];
    const float dx = victim.move.origin.x - e.move.origin.x;
    const float dz = victim.move.origin.z - e.move.origin.z;
    if (e.weapon.reload_ticks > 0U && nav_cover(s, index, eye_of(victim))) {
      bot.cover_ticks = e.weapon.reload_ticks + 32U;
    } else if (dx * dx + dz * dz > (kPreferredRange + kRangeSlack) * (kPreferredRange + kRangeSlack) ||
               std::fabs(feet_of(victim).y - feet_of(e).y) > kStepHeight) {
      const float gx = victim.move.origin.x - bot.goal.x;
      const float gz = victim.move.origin.z - bot.goal.z;
      if (bot.path_cursor == bot.path_count || gx * gx + gz * gz > 128.0F * 128.0F ||
          std::fabs(victim.move.origin.y - bot.goal.y) > 48.0F) {
        bot.goal = victim.move.origin;
        nav_plan(s, index, false);
      }
    } else {
      bot.path_count = bot.path_cursor = 0U;
    }
  }
  Vec3 waypoint = e.move.origin;
  bool following = nav_waypoint(e, waypoint);
  if (!has_target && bot.cover_ticks == 0U) {
    if (bot.goal_ticks > 0U) --bot.goal_ticks;
    if (bot.goal_ticks == 0U || !following) {
      pick_goal(s, index);
      following = nav_waypoint(e, waypoint);
    }
  }
  if (has_target) {
    const PlayerEntity& victim = s.players[bot.target];
    ...
    const float dx = waypoint.x - e.move.origin.x;
    const float dz = waypoint.z - e.move.origin.z;
    if (following) {
  ...
  if (following) {
    want_x = waypoint.x - e.move.origin.x;
    want_z = waypoint.z - e.move.origin.z;
  } else if (bot.cover_ticks > 0U) {
    cmd.buttons |= ButtonDuck;
  } else if (has_target) {
  ...
  if (want_len > 1e-4F) {
    want_x /= want_len;
    want_z /= want_len;
    // Brake into corners instead of carrying full running speed off a ramp.
    if (following && want_len < 32.0F) {
      const float brake = 0.35F + 0.65F * want_len / 32.0F;
      want_x *= brake; want_z *= brake;
    }
  }

  // Pause and replan a blocked route; combat strafing can reverse sideways.
  // Do not probe beyond a waypoint: the path may turn at the pit's edge.
  const float probe_distance = following && want_len < 34.0F ? want_len : 34.0F;
  const bool blocked_ahead = want_len > 1e-4F && !footing_ahead(e, want_x, want_z, probe_distance);
  ...
  if (blocked_ahead || stuck) {
    want_x = following ? 0.0F : right_x * bot.strafe_dir;
    want_z = following ? 0.0F : right_z * bot.strafe_dir;
    if (!footing_ahead(e, want_x, want_z)) want_x = want_z = 0.0F;
    if (stuck) {
      if (bot.stuck_ticks > kStuckLimit * 3U) {
        bot.stuck_ticks = 0U;
        bot.strafe_dir = -bot.strafe_dir;
        if (!has_target) pick_goal(s, index);
        else bot.navigation_ticks = 0U;
      }
    }
  }
